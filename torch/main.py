...
class AeroHeatingAnalysis:
    ...
    time_data : list
        List of times that the altitude and velocity data corresponds to (s).
    altitude_data : list
        Altitude at each time (m)
    velocity_data : list
        Velocity at each time (m/s)
    ...
    time_data : list
        Time data array (s)
    altitude_data : list
        Altitude data array (m)
    velocity_data : list
        Velocity data array (m/s)
    fixed_wall_temperature : bool
        If True, the wall temperature is fixed to its starting value. Otherwise a simple model is used to model its temperature change.
    turbulent_transition_Rex : float, optional
        Local Reynolds number at which the boundary layer transition from laminar to turbulent. Defaults to 7.5e6.
    ...
    M : numpy ndarray
        Local Mach number at each station and timestep. Has dimensions (15, N) where N is the length of the "time" array.
    P : numpy ndarray
        Local static pressure at each station and timestep. Has dimensions (15, N) where N is the length of the "time" array.
    Te : numpy ndarray
        Temperature at the edge of the boundary layer, at each station and timestep. Has dimensions (15, N) where N is the length of the "time" array.
    Tw : numpy ndarray
        Wall temperature at each timestep. Local static pressure at each station and timestep. Has dimensions (N) where N is the length of the "time" array.
    Trec_lam : numpy ndarray
        Adiabatic wall tempearture (also known as the 'recovery temperature') for a laminar boundary layer, at each station and timestep. Has dimensions (15, N) where N is the length of the "time" array.
    Trec_turb : numpy ndarray
        Adiabatic wall tempearture (also known as the 'recovery temperature') for a turbulent boundary layer, at each station and timestep. Has dimensions (15, N) where N is the length of the "time" array.
    Hstar_function : numpy ndarray
        Function that needs to be integrated to get H*(x), as defined in the NASA documents. This is not equal to H*(x) itself. This is stored because it's needed for integration. Has dimensions (15, N) where N is the length of the "time" array.
    Rex : numpy ndarray
        Local Reynolds number at each station and timestep. Has dimensions (15, N) where N is the length of the "time".
    
    q_lam : numpy ndarray
        Heat transfer rate (W/m^2) with a laminar boundary layer, at each station and timestep. Has dimensions (15, N) where N is the length of the "time".
    q_turb : numpy ndarray
        Heat transfer rate (W/m^2) with a turbulent boundary layer, at each station and timestep. Has dimensions (15, N) where N is the length of the "time".
    q0_hemispherical_nose : numpy ndarray
        Heat transfer rate (W/m^2) at the stagnation point at each timestep, if we were using a hemispherical nosecone. Has dimensions (N) where N is the length of the "time" array.
    ...
    def __init__(self, 
        ...
        self.M = np.full([15, len(self.time_data)], float("NaN"))        #Local Mach number
        self.P = np.full([15, len(self.time_data)], float("NaN"))        #Local pressure

        #Initialise the wall temperature:
        if starting_temperature == None:
            starting_temperature = Atmosphere(self.altitude_data[0]).temperature[0]           #Assume the nose cone starts with ambient temperature

        if self.fixed_wall_temperature == True:
            self.Tw = np.full(len(self.time_data), starting_temperature)
        else:
            self.Tw = np.full(len(self.time_data), float("NaN"))                                          
            self.Tw[0] = starting_temperature

        self.Te = np.full([15, len(self.time_data)], float("NaN"))                                         #Temperature at the edge of the boundary layer
        self.Tstar = np.full([15, len(self.time_data)], float("NaN"))                                      #T* as defined in the paper
        self.Trec_lam = np.full([15, len(self.time_data)], float("NaN"))                                   #Temperature corresponding to hrec_lam
        self.Trec_turb = np.full([15, len(self.time_data)], float("NaN"))                                  #Temperature corresponding to hrec_turb
        self.Hstar_function = np.full([15, len(self.time_data)], float("NaN"))                             #This array is used to minimise number of calculations for the integration needed in H*(x)
        self.Rex = np.full([15, len(self.time_data)], float("NaN"))                                        #Local Reynolds number

        #Arrays to store the heat transfer rates
        self.q_lam = np.full([15, len(self.time_data)], float("NaN"))                     #Laminar boundary layer
        self.q_turb = np.full([15, len(self.time_data)], float("NaN"))                    #Turbunulent boundary layer
        self.q0_hemispherical_nose = np.full(len(self.time_data), float("NaN"))     #At the stagnation point for a rocket with a hemispherical nose cone - used as a reference point
    ...
    def step(self, print_style=None):
        ...
        #Get altitude:
        alt = self.altitude_data[self.i]

        #Ambiance data is capped to 81.020 km altitude - so just use atmospheric properties at 81.020 km if we fly above that.
        if alt > 81020:
            alt = 81020         #Altitude isn't used for anything except atmospheric properties, so it's okay for us to do this.
        ...
        Vinf = np.linalg.norm(self.velocity_data[self.i])
        ...
        if Minf > 1:
            ...
            if oblique_MS > 1:
                ...
                if self.fixed_wall_temperature == False:
                    ...
                    Q_tot = Qdot_tot * (self.time_data[self.i+1] - self.time_data[self.i])  #Q = ∫Qdot dt, using left Riemann sum
    ...
    def run(self, number_of_steps = None, starting_index = 0, print_style="minimal"):
        ...
        if number_of_steps == None:
            number_of_steps = len(self.time_data) - 1 - starting_index
    ...
    def plot_station(self, station_number=10, imax=None):
        ...
        imax : int
            Maximum index (of the time array) to plot to.
        """
    ...
    def plot_heat_transfer(self, i=0, y_limit_scaling=1.5, automatic_rescaling=True):
        """
        Plots heat transfer along the nosecone. Has an interactive slider that you can use to browse through different indexes of the time array.
        Inputs
        -------
        i : int
            The time array index to start the plot at. Defaults to 0.
        ...
        alt = self.velocity_data[i]
        Vinf = self.velocity_data[i]
        Minf = Vinf/Atmosphere(alt).speed_of_sound[0]
        fig.suptitle('i={} time = {:.2f} s alt={:.2f} km Minf={:.2f}'.format(i, self.time_data[i], alt/1000, Minf))
        ...
        slider = matplotlib.widgets.Slider(slider_axis, 'Index', 0, len(self.time_data)-1, valinit=initial_value)
        ...
        def update(value):
            ...
            #Recreate the title
            Vinf = self.velocity_data[index]
            Minf = Vinf/Atmosphere(alt).speed_of_sound[0]
            fig.suptitle('i={} time = {:.2f} s alt={:.2f} km Minf={:.2f}'.format(index, self.time_data[index], alt/1000, Minf))
